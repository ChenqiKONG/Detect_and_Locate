...
class TemplateMap(nn.Module):
    def __init__(self, c_in, templates, mean_template):
...
if __name__ == '__main__':
    MODEL = Model(maptype='reg', templates='tmp', num_classes=2, load_pretrain=True)
    faces = torch.rand(size=(8, 3, 299, 299))
    x, mask, vec = MODEL.model(faces)
    print(x.size(), mask.size(), type(vec))
